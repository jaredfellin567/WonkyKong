...
class GraphObject
{
  public:
	...
	GraphObject(int imageID, int startX, int startY, int dir = 0, double size = 1.0)
	...
	void getPositionInThisDirection(int dir, int units, int& newX, int& newY) const
	{
		...
		switch (dir)
		{
		  case right:  newX += units; break;
		  case left:   newX -= units; break;
		  case up:     newY += units; break;
		  case down:   newY -= units; break;
		}
